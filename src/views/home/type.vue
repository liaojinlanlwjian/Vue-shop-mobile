				...
				<template #content>
					<van-row gutter="20">
						<van-col style="width:100px; height: 100px; margin-left: 20px; margin-top: 20px;"
							v-if="active === 0" v-for="(item ,index) in goodsList_moblie" :key="index" span="8"
							@click="focu(item)">
							<van-image width="6rem" height="6rem" fit="fill" :src="item.image_src" />
						</van-col>
						<van-col v-if="active === 1" v-for="(item ,index) in goodsList_nvzhuan" :key="index" span="8"
							@click="focu(item)">
							<van-image width="4rem" height="6rem" fit="fill" :src="item.image_src" />
						</van-col>
						<van-col v-if="active === 2" v-for="(item ,index) in goodsList_meizhuan" :key="index" span="8"
							@click="focu(item)">
							<van-image width="6rem" height="6rem" fit="fill" :src="item.image_src" />
						</van-col>
						<van-col v-if="active === 3" v-for="(item ,index) in goodsList_diannao" :key="index" span="8"
							@click="focu(item)">
							<van-image width="6rem" height="6rem" fit="fill" :src="item.image_src" />
						</van-col>
						<van-col v-if="active === 4" v-for="(item ,index) in goodsList_zhuzhu" :key="index" span="8"
							@click="focu(item)">
							<van-image width="6rem" height="6rem" fit="fill" :src="item.image_src" />
						</van-col>
					</van-row>
